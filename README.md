...
2. Buat API Key: **Settings → Developers → API Keys → Create secret key** → isi Name mis. `SPMB Dev` → pada tabel **Permissions**, set **Money-in products = Write**, **Money-out products = Read**, **Balance = Read**, selebihnya **None** → **Create** → salin `xnd_development_...` → tempel ke `XENDIT_API_KEY`.

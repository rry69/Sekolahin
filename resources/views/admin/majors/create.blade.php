    ...
    <div class="mjr-card-body">
      <div class="mjr-grid">
        ...
        <div class="mjr-field">
          ...
          <input type="text" name="name" value="{{ old('name') }}" required maxlength="255" placeholder="e.g. Rekayasa Perangkat Lunak" class="mjr-input">
          @error('name')<p class="mjr-err">{{ $message }}</p>@enderror
        </div>

        <div class="mjr-field">
          <label>Kode <span class="req">*</span></label>
          <input type="text" name="code" value="{{ old('code') }}" required maxlength="50" placeholder="e.g. RPL" class="mjr-input" style="text-transform:uppercase;">
        ...
        <div class="mjr-field">
          ...
          <input type="number" name="order" value="{{ old('order') }}" min="0" class="mjr-input" placeholder="e.g. 1">
          ...
          <div class="mjr-quota-box">
            <div class="mjr-grid" style="grid-template-columns:repeat(3,1fr);">
              @foreach($tracks as $t)
                <div class="mjr-field">
                  <label>{{ $t->name }}</label>
                  <input type="number" name="quota_track_{{ $t->id }}" value="{{ old('quota_track_'.$t->id) }}" min="0" class="mjr-input quota-input" data-track="{{ $t->id }}" placeholder="0">
        ...
        <div class="mjr-field full">
          ...
          <textarea name="description" rows="3" class="mjr-input">{{ old('description') }}</textarea>
      ...
      <div class="mjr-submit">
        ...
        <button type="submit" class="btn btn-primary" id="saveBtn">
          <i class="fa-solid fa-floppy-disk" style="font-size:11px;"></i> <span id="saveBtnText">Tambah Jurusan</span>
        </button>
      </div>
    </div>
  </div>

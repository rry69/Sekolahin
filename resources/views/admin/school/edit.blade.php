...
<style>
  ...
  .skl-logo-wrap { display: flex; align-items: center; gap: 20px; flex-wrap: wrap; }
  ...
  .skl-logo-preview img { max-width: 100%; max-height: 100%; object-fit: contain; }
...
<div style="display:flex;justify-content:space-between;align-items:flex-start;gap:16px;flex-wrap:wrap;margin-bottom:18px;">
  <div>
    <h1 class="page-title" style="margin-bottom:2px;">Edit Sekolah</h1>
    <p style="font-size:13px;color:var(--tx2);">Kelola profil sekolah yang ditampilkan pada form pendaftaran siswa.</p>
  </div>
...
<form action="{{ route('admin.schools.update', $school) }}" method="POST" enctype="multipart/form-data" id="schoolForm">
  ...
  <div class="skl-card">
    ...
    <div class="skl-grid">
      <div class="skl-field">
        ...
        <input type="text" name="name" value="{{ old('name', $school->name) }}" required maxlength="255" class="skl-input">
        @error('name')<p class="skl-err">{{ $message }}</p>@enderror
      </div>

      <div class="skl-field">
        <label>NPSN <span class="req">*</span></label>
        <input type="text" name="npsn" value="{{ old('npsn', $school->npsn) }}" required maxlength="8" inputmode="numeric" pattern="[0-9]{8}" title="NPSN harus 8 digit angka" class="skl-input" id="npsnInput">
      ...
      <div class="skl-field full">
        ...
        <input type="text" name="principal_name" value="{{ old('principal_name', $school->principal_name) }}" maxlength="255" class="skl-input">
  ...
  <div class="skl-card">
    ...
    <div class="skl-grid">
      <div class="skl-field">
        <label>Telepon</label>
        <input type="text" name="phone" value="{{ old('phone', $school->phone) }}" maxlength="50" class="skl-input" inputmode="numeric">
        @error('phone')<p class="skl-err">{{ $message }}</p>@enderror
      </div>

      <div class="skl-field">
        <label>WhatsApp</label>
        <input type="text" name="whatsapp" value="{{ old('whatsapp', $school->whatsapp) }}" maxlength="50" class="skl-input" inputmode="numeric" placeholder="08xxxxxxxxxx">
        @error('whatsapp')<p class="skl-err">{{ $message }}</p>@enderror
      </div>

      <div class="skl-field">
        <label>Email</label>
        <input type="email" name="email" value="{{ old('email', $school->email) }}" maxlength="255" class="skl-input">
        @error('email')<p class="skl-err">{{ $message }}</p>@enderror
      </div>

      <div class="skl-field">
        <label>Website Sekolah</label>
        <input type="url" name="website" value="{{ old('website', $school->website) }}" maxlength="255" class="skl-input" placeholder="https://..." >
  ...
  <div class="skl-card">
    ...
    <div class="skl-grid">
      <div class="skl-field full">
        ...
        <textarea name="address" rows="2" class="skl-input">{{ old('address', $school->address) }}</textarea>
        @error('address')<p class="skl-err">{{ $message }}</p>@enderror
      </div>

      <div class="skl-field">
        <label>Kecamatan</label>
        <input type="text" name="district" value="{{ old('district', $school->district) }}" maxlength="255" class="skl-input">
        @error('district')<p class="skl-err">{{ $message }}</p>@enderror
      </div>

      <div class="skl-field">
        <label>Kota/Kabupaten</label>
        <input type="text" name="city" value="{{ old('city', $school->city) }}" maxlength="255" class="skl-input">
        @error('city')<p class="skl-err">{{ $message }}</p>@enderror
      </div>

      <div class="skl-field">
        <label>Provinsi</label>
        <input type="text" name="province" value="{{ old('province', $school->province) }}" maxlength="255" class="skl-input">
        @error('province')<p class="skl-err">{{ $message }}</p>@enderror
      </div>

      <div class="skl-field">
        <label>Link Google Maps</label>
        <input type="url" name="maps_link" value="{{ old('maps_link', $school->maps_link) }}" maxlength="255" class="skl-input" placeholder="https://maps.google.com/...">
        <p class="skl-hint">Opsional. Harus diawali http:// atau https://</p>
  ...
  <div class="skl-card">
    ...
    <div class="skl-grid">
      <div class="skl-field full">
        ...
        <div class="skl-logo-wrap">
          <div class="skl-logo-preview" id="logoPreview">
            @if ($logoUrl)
          ...
          <div class="skl-logo-actions">
            ...
            @if ($school->logo_path)
              <label class="skl-check" style="border-color:var(--badge-rejected-fg);">
      ...
      <div class="skl-field full">
        ...
        <textarea name="description" rows="3" maxlength="500" class="skl-input" id="descInput">{{ old('description', $school->description) }}</textarea>
  ...
  <div class="skl-card">
    <h4 class="skl-card-title"><i class="fa-solid fa-graduation-cap"></i> Jenjang yang Dilayani</h4>
...
(function () {
  ...
  // --- Sinkronkan checkbox hapus logo dengan preview ---
  if (removeLogoCheck) {

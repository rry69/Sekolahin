...
<div class="py-12">
    <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6">
                <h3 class="text-2xl font-bold text-gray-900 mb-6">Edit Periode Pendaftaran</h3>

                <form action="{{ route('admin.periods.update', $registrationPeriod) }}" method="POST">
                    @csrf
                    @method('PATCH')
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="name" value="{{ old('name', $registrationPeriod->name) }}" required class="w-full border-gray-300 rounded-md shadow-sm">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="max_applicants" value="{{ old('max_applicants', $registrationPeriod->max_applicants) }}" min="1" class="w-full border-gray-300 rounded-md shadow-sm">
                        @error('max_applicants')<p class="text-red-600 text-sm mt-1">{{ $message }}</p>@enderror
                    </div>

                    <div class="mb-6">
                        <label class="inline-flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $registrationPeriod->is_active) ? 'checked' : '' }} class="rounded border-gray-300 text-blue-600">
                            <span class="text-sm text-gray-700">Periode aktif</span>
                        </label>
                    </div>
